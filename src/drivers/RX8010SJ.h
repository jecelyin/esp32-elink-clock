...
struct DateTime {
  ...
  uint8_t minute;
  uint8_t hour;
  uint8_t dayOfWeek; // Has to be set
  uint8_t dayOfMonth;
  uint8_t month;
  uint8_t year; // Two digits
};

class Adapter {
public:
  // Constructor/destructor
  Adapter(uint8_t i2cSlaveAddr);
  virtual ~Adapter();
  ...
  void setFCTCounter(uint16_t multiplier, uint8_t frequency);
  uint16_t getFCTCounter();
  void setFCTOutput(uint8_t pin);
  void enableFCT();
  void disableFCT();
  bool checkFCT();

  // Alarm
  void setAlarm(DateTime time, uint8_t mode);
  void enableAlarm();
  void disableAlarm();
  bool checkAlarm();

  // Time Update Interrupt
  void setTUIMode(uint8_t mode);
  void enableTUI();
  void disableTUI();
  bool checkTUI();

  // Frequency output
  void enableFOUT(uint8_t frequency, uint8_t pin);
  void disableFOUT();

private:
  uint8_t i2cAddress;

  uint8_t readFromModule(uint8_t address);
  void writeToModule(uint8_t address, uint8_t data);
  void writeFlag(uint8_t address, uint8_t pos, uint8_t value);
  uint8_t getSingleBit(uint8_t binary);
  uint8_t getValueFromBinary(uint8_t binary, uint8_t pos);
  uint8_t getValueFromBinary(uint8_t binary, uint8_t pos, uint8_t val);
  uint8_t sumValueFromBinary(uint8_t binary, uint8_t length);
  uint8_t setEightyBinary(uint8_t binary, uint8_t val);
  uint8_t setFortyBinary(uint8_t binary, uint8_t val);
  uint8_t setTwentyBinary(uint8_t binary, uint8_t val);
  uint8_t setTenBinary(uint8_t binary, uint8_t val);
  uint8_t setBinary(uint8_t binary, uint8_t pos, uint8_t flagVal);
};
} // namespace RX8010SJ
